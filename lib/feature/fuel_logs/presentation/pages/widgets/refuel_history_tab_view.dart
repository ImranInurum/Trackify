...
class RefuelHistoryTabView extends StatefulWidget {
  const RefuelHistoryTabView({super.key});
          ...
          return SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                RefuelSummaryGrid(
                  totalFuel: state.totalFuelAdded,
                  totalSpendings: state.totalSpendings,
                  avgMileage: state.averageMileage,
                  refuelCount: state.refuelCount,
                ),
                ...
                _buildRefuelLogs(
                  context,
                  l10n,
                  _applyFilters(state.refuelLogs),
                ),
      ...
      case 'leastExpensive':
        filtered.sort((a, b) =>
            (double.tryParse(a.amount) ?? 0)
                .compareTo(double.tryParse(b.amount) ?? 0));
        break;
      case 'bestMileage':
        filtered.sort((a, b) =>
            (double.tryParse(b.mileage ?? '0') ?? 0)
                .compareTo(double.tryParse(a.mileage ?? '0') ?? 0));
        break;
      case 'worstMileage':
        filtered.sort((a, b) =>
            (double.tryParse(a.mileage ?? '0') ?? 0)
                .compareTo(double.tryParse(b.mileage ?? '0') ?? 0));
        break;
    }
    ...
    return Column(
      children: logs.asMap().entries.map((entry) {
        ...
        return RefuelLogListItem(
          ...
          liters: log.liters,
          mileage: log.mileage,
          showDetails: index == 0, // Show details for first item as per design
        );
      }).toList(),
    );
  }
